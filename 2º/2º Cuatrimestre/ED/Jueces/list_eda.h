...
protected:
    ...
    public:
        ...
        IteradorR() : act(nullptr), fan(nullptr) {}
        ...
        IteradorR& operator++() { // ++ prefijo (recomendado)
            if (act == fan) throw std::out_of_range("fuera de la lista");
            act = act->ant;
            return *this;
        }

        IteradorR operator++(int) { // ++ postfijo
            IteradorR ret(*this);
            operator++();
            return ret;
        }

        bool operator==(IteradorR const& that) const {
            return act == that.act && fan == that.fan;
        }
    ...
    private:
        ...
        // constructora privada
        IteradorR(Nodo* ac, Nodo* fa) : act(ac), fan(fa) {}
    };
...
public:
    ...
    iterator insert(iterator const& it, T const& elem) {
        ...
        Nodo* nuevo = this->inserta_elem(elem, it.act, it.act->sig);
    ...
    using reverse_iterator = IteradorR<T>;
